...
int UPLogin::ClientRequest(string &sPrivateKey,Json::Value &root)
{
    ...
    int ret=RSA_private_encrypt(JSON_LEN,from,to,rsa,RSA_NO_PADDING);
    if(ret<0)return -5;
	BIO_free(bio);
	RSA_free(rsa);
...
// return <0: failed, 0: success
int UPLogin::ServerCheck(Json::Value &root,string &sPublicKey)
{
    enum{JSON_LEN=RSA_KEY_BYTES};
