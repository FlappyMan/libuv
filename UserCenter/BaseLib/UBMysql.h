...
public:
	...
	// 在内部sql缓冲中构造sql
	bool Sql(string &sSql,bool bCleanBuffer=false){return Sql(sSql.data(),sSql.length(),bCleanBuffer);};
	bool Sql(const char *pSql, bool bCleanBuffer=false){return Sql(pSql,strlen(pSql),bCleanBuffer);};
	bool Sql(const char *pSql,int iSqlLen,bool bCleanBuffer=false);
