...
void TestCanceltradebatch()
{
    UPCanceltradebatch cancel;
    for(int i = 0; i < 6; ++i)
    {
        cancel.add_ids(i);
    }
    const int BUFFSIZE=1024;
    char buffer[BUFFSIZE];
    int iLen=JsonPack<UPCanceltradebatch>(&cancel,buffer,BUFFSIZE);
    buffer[iLen]='\0';
    cout<<buffer<<endl;

    UPCanceltradebatch cancel2;
    JsonUnpack<UPCanceltradebatch>(&cancel2,buffer,iLen);
    for(uint32_t i = 0; i < cancel2.ids_size(); ++i)
    {
